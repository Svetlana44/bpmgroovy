 ...
 * 
 * ОСНОВНЫЕ ОТЛИЧИЯ ОТ REDIS:
 * ===========================
 * 
 * MongoDB:
 * - Хранит данные на диске (с опциональным кэшем в памяти)
 * - Документо-ориентированная БД (JSON-подобные документы)
 * - Поддерживает сложные запросы и агрегации
 * - Используется для хранения документов, каталогов, логов
 * - Идеально для проверки данных в тестах (подготовка/валидация)
 * 
 * Redis:
 * - Хранит данные в памяти (RAM) - очень быстро
 * - Key-value хранилище
 * - Ограниченные возможности запросов
 * - Используется для кэширования, сессий, очередей
 * - Идеально для кэширования в тестах, моков
 * 
 * В ТЕСТАХ:
 * - MongoDB: проверка сохраненных данных, подготовка тестовых данных
 * - Redis: кэширование результатов, мокирование внешних сервисов
 * 
 * См. также: ../../comparisons/MongoDB_vs_Redis.md для подробного сравнения
 * 
 * Основные операции:
 * - Create (Insert)
 * - Read (Find)
...
public class MongoDBExampleTest {
     ...
     * 
     * В MongoDB документы хранятся в коллекциях (аналог таблиц в SQL).
     * Документы - это JSON-подобные структуры, что позволяет хранить
     * сложные вложенные данные.
     * 
     * В отличие от Redis (key-value), MongoDB позволяет хранить
     * структурированные документы с множеством полей.
     */
    @Test
     ...
     * 
     * MongoDB поддерживает богатый язык запросов, в отличие от Redis,
     * где запросы ограничены операциями с ключами.
     */
    @Test
     ...
     * 
     * MongoDB поддерживает сложные фильтры, что невозможно в Redis
     * (Redis работает только с ключами, без условий на значения).
     */
    @Test
    void testFindWithFilter() {
        ...
        // В Redis это невозможно - нужно получать все данные и фильтровать в коде
     ...
     * 
     * MongoDB поддерживает сложные запросы с множественными условиями,
     * что невозможно в Redis (Redis работает только с ключами).
     * 
     * В Redis для таких запросов нужно:
     * 1. Получить все ключи (keys *)
     * 2. Получить значения для каждого ключа
     * 3. Фильтровать в коде приложения
     * 
     * В MongoDB это делается одним запросом на уровне БД.
     */
    @Test
