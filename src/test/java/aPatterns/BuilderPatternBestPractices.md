...
Документация описывает лучшие практики использования паттерна Builder в тестовом фреймворке, включая улучшения для классов User и централизацию создания тестовых пользователей.
...
- Пароли и имена пользователей захардкожены в коде
- Нет централизованного управления тестовыми данными
- Сложно изменить значения для всех тестов одновременно

### 3. **Проблемы с Builder паттерном**

- **Публичные поля** вместо приватных с геттерами
- **Нет валидации** обязательных полей
- **Нет factory методов** для стандартных пользователей
- **Нет fluent API** для часто используемых комбинаций
...
### Шаг 2: Обновить UIBaseTests

Аналогично обновить создание пользователей в `UIBaseTests.java`

### Шаг 3: Обновить другие тестовые классы

Найти все места создания пользователей и заменить на использование `TestUserFactory`
...
Использование `TestUserFactory` и улучшенного Builder паттерна:

- ✅ Устраняет дублирование кода
- ✅ Улучшает читаемость тестов
- ✅ Централизует управление тестовыми данными
- ✅ Использует значения из properties
- ✅ Следует лучшим практикам Java и паттернов проектирования
